...
func _ready():
	...
	# Dumb loading
	$PLYLoader.load_ply(splat_filename)
	...
	var fdc_0 = $PLYLoader.get_vertex_property("f_dc_0")
	var fdc_1 = $PLYLoader.get_vertex_property("f_dc_1")
	var fdc_2 = $PLYLoader.get_vertex_property("f_dc_2")
	...
	for i in range(num_vertex):
		positions.append_array([x[i], y[i], z[i], 0])
		opacities.append(opacity[i])
		scales.append_array([scale_0[i], scale_1[i], scale_2[i], 0])
		rotations.append_array([rot_0[i], rot_1[i], rot_2[i], rot_3[i]])
		depth_index.append(i)
		depths.append(0)
		
		var coeff = [fdc_0[i], fdc_1[i], fdc_2[i]]

		for j in range(num_coeffs_per_color):
			coeff.append(coeffs[0 * num_coeffs_per_color + j][i])
			coeff.append(coeffs[1 * num_coeffs_per_color + j][i])
			coeff.append(coeffs[2 * num_coeffs_per_color + j][i])
	...
	var shader_file = load("res://splat.glsl")
